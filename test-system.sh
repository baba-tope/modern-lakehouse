...
echo "------------------"
test_service "Fuel Sales Data" "kubectl exec postgres-0 -n $NAMESPACE -- psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc 'SELECT COUNT(*) FROM analytics.fuel_sales' | grep -E '^[1-9][0-9]+$'"
test_service "Store Sales Data" "kubectl exec postgres-0 -n $NAMESPACE -- psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc 'SELECT COUNT(*) FROM analytics.store_sales' | grep -E '^[1-9][0-9]+$'"
test_service "Stations Data" "kubectl exec postgres-0 -n $NAMESPACE -- psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc 'SELECT COUNT(*) FROM analytics.stations' | grep -E '^[1-9][0-9]*$'"
test_service "dbt Fact Table" "kubectl exec postgres-0 -n $NAMESPACE -- psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc 'SELECT COUNT(*) FROM analytics_mart.fct_daily_sales' | grep -E '^[1-9][0-9]+$'"

echo ""
echo "3. Database Objects"
echo "-------------------"
test_service "Analytics Schema" "kubectl exec postgres-0 -n $NAMESPACE -- psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc '\dn' | grep analytics"
test_service "Staging Views" "kubectl exec postgres-0 -n $NAMESPACE -- psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc '\dv analytics_staging.*' | grep stg_"
test_service "Mart Schema" "kubectl exec postgres-0 -n $NAMESPACE -- psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc '\dn' | grep analytics_mart"
